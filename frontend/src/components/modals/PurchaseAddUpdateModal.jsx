...
const purchaseModalSchema = object().shape({
  firm_id: number().required("You must select a firm"),
...
export default function PurchaseAddUpdateModal({
  open,
  handleClose,
  info,
  setInfo,
    ...
    <div>
      ...
      >
        <Box sx={modalStyle(isDark)}>
          <Formik
            initialValues={info}
            validationSchema={purchaseModalSchema}
            ...
            {({
              ...
              >
                <Field
                  ...
                  onBlur={handleBlur}
                  error={touched?.firm_id && Boolean(errors?.firm_id)}
                  helperText={touched?.firm_id && errors?.firm_id}
                  required
                >
                  {firms?.map((item) => (
